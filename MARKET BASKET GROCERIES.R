...
library(arules)  #install first data of frequently brought
...
frequentItems <- eclat (Groceries, parameter = list(supp = 0.005, minlen= 1, maxlen = 5)) #supp=a union b divide by total no of transaction . a ca be a 1 item or a set of item .frequently purchased =support value 0.0005
frequentItems
...
rules <- apriori(Groceries, parameter = list(supp = 0.005, conf = 0.5)) #confidence = NUMBER OF TIMES B WAS BOUGHT WHEN A WAS BOUGHT FRST.THE REST OF THE VALUES , a WAS BOUGHT BUT SOMETHING ELSE WAS BOUGHT AFTER THAT. #SUPPOT= NUMBER OF TIMES BOTH THE ITEMS A . SUPPORT IS TRANSITIVE . WHEN A IS BOUGHT FRST OR B IS BOUGHT FRST , IT DOESNT MATTER. IT MATTERS ONLY IJN CONFIDENCE
rules
